...
// Associated functions that are defined on a type generally
...
// Implement block, all `Point` associated functions are methods go in here
impl Point {
...
impl Rectangle {
    // This is a method
    // `&self` is sugar for `self: &Self` where `Self` is the type of the caller
    // object. In this case `Se;f` = `Rectangle`
    fn area(&self) -> f64 {
    ...
    // This method required the caller object to mutable
...
fn main() {
    ...
    // Unlike rectangle object, mutable objects can call mutable methods
